...
class MultiCrops extends Component {
  ...
  handleMouseMove = e => {
    ...
    if (isValidPoint(pointA)) {
      ...
      const coordinate = {
        ...
        width: Math.abs(pointA.x - pointB.x) >= 102 ? 102 : Math.abs(pointA.x - pointB.x),
        height: Math.abs(pointA.y - pointB.y) >= 102 ? 102 : Math.abs(pointA.y - pointB.y),
        id: this.id,
      };
  ...
  handleMouseUp = e => {
    const { onDrawEnd, coordinates } = this.props;
    const { pointA } = this;
    if (isValidPoint(pointA)) {
      const pointB = this.getCursorPosition(e);
      ...
      const nextCoordinates = clone(coordinates);
      nextCoordinates[this.drawingIndex] = coordinate;
      if (is(Function, onDrawEnd)) {
        onDrawEnd(coordinate, this.drawingIndex, nextCoordinates);
      }
      this.pointA = {};
    }
  };
