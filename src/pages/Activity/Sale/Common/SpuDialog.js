...
import { addSpusColumns } from './Detail/ActivityForm.config';
...
class SpuDialog extends React.Component {
  ...
  loadValidSpus = async (current = 1, size = 10, spuSearchWord) => {
    ...
    if (res.data && res.data.records && !_.isEmpty(res.data.records)) {
      res.data.records.forEach(ele => {
        ele.blackUrl = ele.skuImg || ele.blackUrl;
        ele.englishName = ele.englishName || ele.spuName;
        // if (ele.blackUrl.indexOf('[') !== -1 && ele.blackUrl.indexOf(']') !== -1) {
        //   ele.blackUrl = JSON.parse(ele.blackUrl)[0];
        // }
      });
    }
  ...
  render() {
    ...
    const spuListLoading = loading.effects['activity/getSpus'];
    return (
      <Modal footer={null} title="添加商品" visible={visible} onOk={this.handleCancel} centered width={1128} maskClosable={false} onCancel={this.handleCancel}>
