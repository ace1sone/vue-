...
class TaskData extends React.Component {
  ...
  giveOut = () => {
    ...
    Modal.confirm({
      ...
      content: '是否立即发放名额',
      onOk: async () => {
        const res = await dispatch({ type: 'task/batch', payload: { id: detail.id, type: 'INVITATION' } });
  ...
  render() {
    const { loading, detail, completeResult } = this.props;
        ...
        <Row gutter={24}>
          <Col lg={12} md={12} sm={24}>
            <Form.Item {...formItemLayout} label="购买资格发放数量">
              ...
              {detail.status && detail.status !== 'WAITING' && detail.issuanceMethod === 'MANUAL' && completeResult !== 'DRAW_CODE' && (
