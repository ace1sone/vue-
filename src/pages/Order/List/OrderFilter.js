    ...
    return (
      <div className={styles['orderFilter-box']}>
        <div>
          ...
          <RangePicker style={{ width: 360 }} onChange={this.time} className={styles['mt-12']} placeholder={['开始时间', '结束时间']} value={time} />
        </div>
        <div style={{ marginTop: 24 }}>
          {orderState === 'allOrder' ? (
            <Select className={styles['margin-24']} placeholder="订单状态" style={{ width: 144 }} onChange={this.paramStatus} value={status}>
              {statusList.map(item => (
                <Option key={item.key} value={item.key}>
                  {item.name}
                </Option>
          ...
          <Button type="primary" style={{ width: 72 }} className={styles['margin-24']} onClick={search}>
            搜索
          </Button>
          <Button style={{ width: 72 }} className={styles['margin-24']} onClick={this.clear}>
            清空
          </Button>
          <Button type="primary" style={{ width: 96, float: 'right' }}>
            导出Excel
          </Button>
        </div>
      </div>
