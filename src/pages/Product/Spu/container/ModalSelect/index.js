...
class ModalSelect extends React.Component {
  ...
  handleSelectChange = (selectedRowKeys, selectedRows) => {
    ...
    if (+newSelectRecord.status === 1) return;
    this.setState({
  ...
  handleRowClick = record => {
    ...
    this.setState({ selectedRowKey: record.id, selected: record });
  };

  render() {
    const { title, onSearch, columns, dataSource, loading, disabled, search, modalProps, tableProps } = this.props;
