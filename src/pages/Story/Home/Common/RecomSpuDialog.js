...
import { recomSpusColumns } from './Detail/StoryForm.config';
...
class SpuDialog extends React.Component {
  ...
  loadValidSpus = async spuSearchWord => {
    ...
    if (spuSearchWord) {
      let filterData = [];
      filterData = res.data.filter(item => item.spuName === spuSearchWord || item.spuId === spuSearchWord);
  ...
  render() {
    ...
    const spuListLoading = loading.effects['story/getRecomSpus'];
    return (
      <Modal
        footer={null}
        title="添加商品"
