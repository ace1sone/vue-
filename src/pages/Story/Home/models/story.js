...
import { getStorys, storyDetail, saveStory, activityOffline, activityOnline } from '@/services/story';
import { getShelves } from '@/services/shelve';
import { setSceneStart } from '@/services/material';
import { getMaterialList } from '@/services/storyMaterial';
...
export default {
  ...
  effects: {
    *fetchList({ payload }, { call, put }) {
      const response = yield call(getStorys, payload);
      yield put({
        type: 'loadList',
        payload: response,
      });
    ...
    *saveStory({ payload, success }, { call }) {
      const response = yield call(saveStory, payload);
